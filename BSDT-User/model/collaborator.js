...
async function getAllInvitations(userId) {
  const { data, error } = await supabase.rpc("get_collab_requests", {
...
async function allprojectviewCollaborator(userId) {
  const { data, error } = await supabase
    .from("project_shared_with_collaborator")
    .select(
      "user_id,access_role, project_id, survey_project(project_id,user_id, title, field, description, user!survey_project_user_id_fkey(user_id, name, email))"
    )
    .eq("user_id", userId)
...
async function getAllSurveyInvitations(userId) {
  ...
  if (error) {
    console.error("Error fetching survey invitations:", error.message);
    return { data: null, error };
  }
...
async function sendSurveyCollaborationRequest(
  survey_id,
  request_email,
  access_role,
  userId
  ...
  try {
    ...
    if (survey.user_id !== userId) {
      return {
        ...
        error: {
          message:
            "You are not authorized to send collaboration requests for this survey.",
          status: 403,
        },
      };
    }
    ...
    if (existingRequestError) {
      console.error(
        "Error checking existing collaboration request:",
        existingRequestError
      );
      return {
        data: null,
        error: {
          message: "Error checking existing collaboration request.",
          status: 500,
        },
      };
    }
    ...
    if (existingRequest) {
      if (existingRequest.invitation === "accepted") {
        return {
          data: null,
          error: {
            message: "Collaboration request already accepted.",
            status: 409,
          },
        };
    ...
    if (error) {
      ...
      return {
        data: null,
        error: { message: "Error sending collaboration request.", status: 500 },
      };
    }
...
async function getSurveyCollaborators(surveyID, userId) {
  ...
  if (survey.user_id !== userId) {
    ...
    return {
      data: null,
      error: {
        message: "You are not authorized to view these collaborators.",
        status: 403,
      },
    };
  }
