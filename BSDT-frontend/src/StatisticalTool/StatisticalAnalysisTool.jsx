...
const StatisticalAnalysisTool = () => {
  ...
  useEffect(() => {
    ...
    if (storedFileName && storedFileURL) {
      ...
      fetch(storedFileURL)
        ...
        .catch((err) => {
          console.error("Could not restore file from session:", err);
          setErrorMessage("Could not restore file. Please re-upload.");
  ...
  const handleFileChange = async (e) => {
    ...
    if (selectedFile) {
      ...
      console.log("File selected, starting upload:", selectedFile.name);
        ...
        .then((data) => {
          if (data.success) {
            ...
            sessionStorage.setItem(
              "fileURL",
              "http://127.0.0.1:8000/" + fixedUrl
            );
          } else {
            setErrorMessage(data.error || "File could not be uploaded.");
            setUploadStatus("error");
          }
        })
        .catch((error) => {
        ...
        .then((data) => {
          if (data.success) {
            ...
            if (fetchedColumns.length > 0) {
              setColumn1(fetchedColumns[0]);
              if (fetchedColumns.length > 1) {
                setColumn2(fetchedColumns[1]); 
              }
            }
          } else {
            setErrorMessage(data.error || "Failed to get columns from file.");
            setUploadStatus("error");
            setColumns([]);
          }
        })
        .catch((error) => {
          console.error("Critical error fetching columns:", error);
          setErrorMessage(
            `Failed to connect or process file: ${error.message}`
          );
          setUploadStatus("error");
          setColumns([]);
        });
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      if (
        ["pearson", "spearman", "cross_tabulation", "cramers_heatmap"].includes(
          testType
        )
      ) {
        formData.append("heatmapSize", heatmapSize);
...
const AnalysisResults = ({
    ...
    return (
      <>
        ...
        {columns?.length > 0 && (
          <p className="mb-3">
            <strong>
              {language === "বাংলা" ? "বিশ্লেষণকৃত কলাম:" : "Columns analyzed:"}
            </strong>{" "}
            {columns
              .filter(Boolean)
              .join(language === "bn" ? " এবং " : " and ")}
          </p>
        )}
                ...
                <div key={index} className="bg-white rounded-lg shadow-md p-4">
                  ...
                  >
                    <div className="relative">
                      <img
                        src={`http://127.0.0.1:8000/${path}`}
                        alt={`${t.tests.mannwhitney} visualization ${
                          index + 1
                        }`}
                        className="w-full h-auto object-contain"
                      />
                      <button
                        onClick={async () => {
                          try {
                            ...
                            const filename =
                              path.split("/").pop() ||
                              `${t.tests.mannwhitney}_visualization_${
                                index + 1
                              }.png`;
                            link.href = url;
                            link.download = filename;
              ...
              <div className="relative">
                ...
                <button
                  onClick={async () => {
                    try {
                      const response = await fetch(
                        `http://127.0.0.1:8000/${results.image_path}`
                      );
                      const blob = await response.blob();
                      const url = window.URL.createObjectURL(blob);
                      const link = document.createElement("a");
  ...
  const renderSpearmanResults = () => {
    ...
    return (
      <>
        <h2 className="text-2xl font-bold mb-4">
          {t.tests.spearman || "Spearman Correlation"}
        </h2>
  ...
  const renderPearsonResults = () => {
    ...
    return (
      <>
        <h2 className="text-2xl font-bold mb-4">
          {t.tests.pearson || "Pearson Correlation"}
        </h2>
  ...
  const renderLinearRegressionResults = () => {
    ...
    return (
      <>
        ...
        <p className="mb-3">
          <strong>
            {language === "bn" ? "কোইফিশিয়েন্ট:" : "Coefficient:"}
          </strong>{" "}
            ...
            <div className="bg-white rounded-lg shadow-md p-4">
              ...
              <button
                onClick={async () => {
                  try {
                    const response = await fetch(
                      `http://127.0.0.1:8000/${results.image_paths[0]}`
                    );
                    const blob = await response.blob();
                    const url = window.URL.createObjectURL(blob);
                    const link = document.createElement("a");
        ...
        {results?.anova_table && (
          <div className="mb-6">
            <h3 className="text-xl font-semibold mb-2">
              {language === "bn" ? "ANOVA টেবিল" : "ANOVA Table"}
            </h3>
            <div
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        const filename =
                          path.split("/").pop() ||
                          `${t.anovaTitle}_visualization_${index + 1}.png`;
                        link.href = url;
                        link.download = filename;
                        document.body.appendChild(link);
                        link.click();
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        const filename =
                          path.split("/").pop() ||
                          `eda_distribution_plot_${index + 1}.png`;
                        link.href = url;
                        link.download = filename;
                        document.body.appendChild(link);
                        link.click();
      ...
      <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white shadow p-4 rounded border">
            <h4 className="text-lg font-semibold mb-2">
              {language === "bn" ? "Chi² পরিসংখ্যান" : "Chi² Statistic"}
            </h4>
            <p>{mapDigitIfBengali(results.statistic?.chi2 || "-")}</p>
          </div>
          <div className="bg-white shadow p-4 rounded border">
            <h4 className="text-lg font-semibold mb-2">
              {language === "bn" ? "P-মান" : "P-value"}
            </h4>
  ...
  const CorrelationHeatmap = ({ data }) => {
    ...
    const variables = [
      ...new Set(data.flatMap((item) => [item.Variable_1, item.Variable_2])),
    ];
      ...
      for (let j = 0; j < variables.length; j++) {
        if (i === j) {
          ...
          const correlation = data.find(
            (item) =>
              (item.Variable_1 === variables[i] &&
                item.Variable_2 === variables[j]) ||
              (item.Variable_1 === variables[j] &&
                item.Variable_2 === variables[i])
          );
          ...
          <thead>
            <tr>
              ...
              {variables.map((variable, idx) => (
                <th
                  key={idx}
                  className="p-2 border bg-gray-100 text-sm font-medium transform -rotate-45 origin-bottom-left h-20"
                >
