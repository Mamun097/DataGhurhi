...
const Data_summary = () => {
  ...
  // NEW: hold original xlsx blob
const [xlsxBlob, setXlsxBlob] = useState(null);

// AOA -> { headers, rows } to feed charts/tables
const aoaToTable = (aoa) => {
...
const activesheetname= sessionStorage.getItem("activesheetname");

useEffect(() => {
  const fetchDirectExcel = async () => {
    if (!FileURL) return;
    setIsLoading(true);
    try {
      ...
      const ws = wb.Sheets[activesheetname] || wb.Sheets[wb.SheetNames[0]];
      const aoa = XLSX.utils.sheet_to_json(ws, { header: 1 });
      setResponses(aoaToTable(aoa));
    } catch (e) {
      console.error("Error fetching direct Excel URL", e);
    } finally {
      setIsLoading(false);
    }
  };
