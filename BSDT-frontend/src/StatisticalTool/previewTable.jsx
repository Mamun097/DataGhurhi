...
const PreviewTable = ({
  ...
  // Utility function to upload edited sheet data as Excel to backend.
  async function uploadEditedSheet(sheet) {
    ...
    // 1. Convert current sheet data to Excel
    ...
    formData.append("file", file, `edited_${sheet.name || "Sheet1"}.xlsx`);
    ...
    try {
      ...
      if (result.success) {
        alert("Changes saved and uploaded successfully!");
      } else {
        alert("Failed to upload Excel: " + result.error);
      }
    } catch (err) {
      console.error("Upload error:", err);
      alert("Failed to upload edited data.");
    }
  }
      ...
      >
        ...
        <div className="mb-2 flex items-center gap-2">
          <button
            ...
            onClick={() => uploadEditedSheet(sheets[activeSheetIndex])}
          >
            Save Changes
          </button>
        </div>
