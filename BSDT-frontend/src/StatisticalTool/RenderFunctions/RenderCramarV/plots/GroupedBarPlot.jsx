...
const GroupedBarPlot = ({
    ...
    const getDefaultSettings = () => {
        ...
        return {
            ...
            yAxisTitle: "Cramer's V", // Default to Cramer's V
            metricType: 'cramers_v', // Default to Cramer's V
            xAxisTitleSize: 16,
            yAxisTitleSize: 16,
    ...
    const generateChartData = () => {
        ...
        const metricType = settings.metricType || 'cramers_v';

        // Create a lookup map for metric values
        ...
        results.pairwise_results.forEach(pair => {
            ...
            if (metricType === 'pvalue' || metricType === 'p_value') {
                ...
                metricValue = typeof pair.cramers_v === 'number' && !isNaN(pair.cramers_v) ? pair.cramers_v : 0;
            } else {
    ...
    const formatValue = (value) => {
        ...
        // For Cramer's V values (0-1 range)
        if (metricType === 'cramers_v') {
            if (value < 0.0001 && value !== 0) {
                return value.toExponential(2);
            }
        ...
        return (
            <div style={{
                ...
                <p style={{ margin: 0, opacity: 0.9, fontSize: '14px' }}>
                    {t('Please run the statistical test first', 'প্রথমে পরিসংখ্যানগত পরীক্ষা চালান')}
                </p>
            </div>
        );
    }
        ...
        <div style={{ position: 'relative', width: '100%' }}>
            ...
            <div style={{
                ...
                <p style={{ margin: 0, color: '#6b7280', fontSize: '14px', lineHeight: '1.6' }}>
                    {t(
                        `This grouped bar chart displays ${settings.metricType === 'pvalue' || settings.metricType === 'p_value' ? 'p-values' : settings.metricType === 'cramers_v' ? "Cramer's V values" : 'Chi-Square statistics'} across different variable comparisons. Each group of bars represents a primary variable, with individual bars showing its relationship strength with other variables. Self-comparisons are automatically excluded.`,
                        `এই গ্রুপড বার চার্টটি বিভিন্ন ভেরিয়েবল তুলনায় ${settings.metricType === 'pvalue' || settings.metricType === 'p_value' ? 'পি-মান' : settings.metricType === 'cramers_v' ? 'ক্রেমারস ভি মান' : 'কাই-স্কয়ার পরিসংখ্যান'} প্রদর্শন করে। প্রতিটি বার গ্রুপ একটি প্রাথমিক ভেরিয়েবল প্রতিনিধিত্ব করে, পৃথক বারগুলি অন্যান্য ভেরিয়েবলের সাথে এর সম্পর্কের শক্তি দেখায়। স্ব-তুলনা স্বয়ংক্রিয়ভাবে বাদ দেওয়া হয়।`
                    )}
                </p>
            </div>
        </div>
