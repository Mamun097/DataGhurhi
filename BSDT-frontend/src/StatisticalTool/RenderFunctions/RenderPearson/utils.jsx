// Utility functions for Chi-Square Results
...
export const downloadBlockPNG = async (anchor, blockRefs) => {
    ...
    link.href = dataURL;
    link.download = `${anchor.replace(/\s+/g, '_')}_spearman_table.png`;
...
export const downloadBlockPDF = async (anchor, blockRefs) => {
    ...
    pdf.save(`${anchor.replace(/\s+/g, '_')}_spearman_table.pdf`);
};
...
export const downloadAllBlocksPDF = async (results, blockRefs) => {
    ...
    pdf.save("spearman_all_comparisons.pdf");
};
...
// Add scatter plot data fetching function
export const fetchScatterData = async (xVariable, yVariable, user_id, df_id, file_url) => {
    try {
        console.log(`Fetching scatter data for: X=${xVariable}, Y=${yVariable}`);
        
        // Get CSRF token
        const csrfToken = getCookie('csrftoken');
        
        const response = await fetch('/api/get_scatter_data/', {
            method: 'POST',
            headers: {
        ...
        if (!data.success) {
            ...
            return {
                success: false,
                error: data.error || 'Failed to fetch scatter data'
            };
        }
        
        console.log(`Successfully fetched ${data.scatter_data?.length || 0} data points`);
        return data;
    } catch (error) {
