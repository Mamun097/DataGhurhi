...
const renderMannWhitneyResults = (mannWhitneyActiveTab, setMannWhitneyActiveTab, results, language, user_id, testType, filename, columns) => {
    ...
    React.useEffect(() => {
        if (results.plot_data && results.plot_data.length > 0) {
            ...
            setRankSettings(prev => ({ ...prev, categoryLabels: labels }));
        }
    }, [results.plot_data]);
    ...
    const getCurrentSettings = () => {
        switch (currentPlotType) {
            ...
            case 'Rank': return rankSettings;
            default: return boxSettings;
        }
    };
    ...
    const setCurrentSettings = (settings) => {
        switch (currentPlotType) {
            ...
            case 'Rank': setRankSettings(settings); break;
        }
    };
    ...
    const renderRankChart = () => {
        ...
        const yDomain = getYAxisDomain(settings, data, 'meanRank');

        return (
            <div style={{ position: 'relative', width: '100%' }}>
                <div style={{ position: 'absolute', top: '10px', right: '10px', display: 'flex', gap: '8px', zIndex: 10 }}>
                    <button className="customize-btn" onClick={() => openCustomization('Rank')}>
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <circle cx="12" cy="12" r="3"></circle>
                            <path d="M12 1v6m0 6v6m9-9h-6m-6 0H3"></path>
                        </svg>
        ...
        <div className="stats-results-container stats-fade-in">
            ...
            <div className="stats-viz-section">
                ...
                <div className="stats-tab-container">
                    ...
                    <button className={`stats-tab ${activeTab === 'rank' ? 'active' : ''}`} onClick={() => setActiveTab('rank')}>{t.rankPlot}</button>
                </div>
...
export default renderMannWhitneyResults;
