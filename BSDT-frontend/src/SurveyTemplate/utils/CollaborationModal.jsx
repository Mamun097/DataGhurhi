...
import { Modal, Button, Form, ListGroup, Spinner, Alert, Row, Col, InputGroup } from "react-bootstrap";
...
const CollaborationModal = ({ show, handleClose, surveyId, surveyTitle }) => {
  ...
  const fetchCollaborators = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || "Failed to fetch collaborators.";
  ...
  const handleRemoveCollaborator = async (collaboratorId) => {
    ...
    try {
        ...
        const errorMessage = err.response?.data?.error || "Failed to remove collaborator.";
        toast.error(errorMessage);
        console.error(err);
    } finally {
        setIsLoading(false);
    ...
    <Modal show={show} onHide={handleClose} centered size="lg" backdrop="static">
      ...
      <Modal.Body>
        ...
        {['pending', 'accepted'].map(status => {
            const list = status === 'pending' ? pendingCollaborators : acceptedCollaborators;
            const title = status === 'pending' ? 'Pending Invitations' : 'Accepted Collaborators';
            const buttonText = status === 'pending' ? 'Cancel' : 'Remove';

            return (
                <div key={status}>
                    <h5 className="mt-4">{title} ({list.length})</h5>
                    {list.length > 0 ? (
                            ...
                            <Row className="align-items-center">
                                <Col>
                                    <div className="text-truncate">
                                        ...
                                        <span className="text-muted d-block d-sm-inline ms-sm-2">({collab.user.email})</span>
                                    </div>
                                    <em className="text-muted">{collab.access_role}</em>
                                </Col>
                                <Col xs="auto">
                                    <Button variant="outline-danger" size="sm" onClick={() => handleRemoveCollaborator(collab.user.user_id)} disabled={isLoading}>
                                        {buttonText}
                                    </Button>
                                </Col>
                            </Row>
