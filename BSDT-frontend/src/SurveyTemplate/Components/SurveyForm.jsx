  ...
  const sendSurveyData = async (url) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        alert(`Survey ${action} successfully!`);
        navigate(
          `/view-survey/${
      ...
      <div className="mb-3">
        {surveyStatus === "published" ? (
          <button onClick={handleUpdate} disabled={isLoading} className="btn btn-outline-primary">
             {isLoading ? (<><span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Updating...</>) : (<><i className="bi bi-pencil"></i> Update</>)}
          </button>
        ) : (
          <>
            <button onClick={handleSave} disabled={isLoading} className="btn btn-outline-secondary me-3">
                {isLoading ? (<><span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Saving...</>) : (<><i className="bi bi-save"></i> Save</>)}
            </button>
            <button onClick={handlePublish} disabled={isLoading} className="btn btn-outline-success">
                {isLoading ? (<><span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Publishing...</>) : (<><i className="bi bi-check-circle"></i> Publish</>)}
            </button>
          </>
        )}
        {surveyLink && (
          <a href={`http://localhost:5173/v/${surveyLink}`} target="_blank" rel="noopener noreferrer" className="btn btn-outline-info ms-3">
